...
4. When frame-locked passes run:
   - Sync main sprites (ENTITIES layer) from the main render queue. With `ySorting`, Pixi keeps last frame's order and reinserts only the slots whose `sortKey` changed, then draws them back to front in one blend. No depth, no second coverage pass, no GPU `sortKey` fallback. Particles, decorations, bullets, Adobe pieces, and liquid join that list. Glow stays a later ADD batch on the `lightGlows` layer when `renderer.lightGlow` is `'add'` (the default). `'sprite'` does not create that layer: the glow quads stay in the entity list, a little in front of the body, and composite source-over (the soft edge can go gray, and the lighting multiply sits on top of them). Custom sprite layers use the same `_uploadSortedSprites` / `orderPainterSlots` path. Without `ySorting` nothing is sorted (bunny). With no painter and lighting off, the upload is dense: no `idxE` walk. The instance vertex buffer is two Pixi buffers swapped each upload, so `bufferSubData` does not hit the buffer the previous draw is still reading. Depth and the old coverage pass stay off: a depth test keeps one winner per pixel and drops the sprite that should show through a soft edge, and on a moving 300k grid that two-pass tied the CPU painter (30.6 ms vs 30.1 ms).
