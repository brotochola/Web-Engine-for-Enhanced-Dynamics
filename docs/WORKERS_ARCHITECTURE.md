...
- **SAB spawn command ring** (MPSC, like Box2D): main and non-owner workers `tryPushSpawn` / `tryPushDespawn`. Slot `entityIndex` is i32. Capacity is `totalEntityCount`. Owner-thread `Type.spawn` / `despawn()` stay in-place — no ring hop (SpawnStorm / BunnySpawner).
- **`config.entityIdWidth` 16 | 32** (default **16**): ECS entity ids in active lists, queries, neighbor/grid slots, and entity Treiber stacks. Width 16 keeps the product ceiling at `MAX_ENTITIES` 65535 (`Uint16`). Width 32 turns on the measured compose **FL4 + PAIR3 + NBR1 + GRID1 + LIST-U32** with ceiling `MAX_ENTITIES_U32` 300000. FL4 is one `Int32` CAS (19-bit index + 13-bit tag) on the entity free-list head; particle / decoration / bullet / joint slot stacks stay `Uint16`. Joint *entity* pairs, decoration parents, and fixture→entity become two `Uint32` columns / `EntityIdArray` when the flag is 32 (leftover hygiene so ids ≥65536 do not alias). Bind once via `bindEntityIdWidth` when SABs are created; do not scatter `=== 32` in hot loops. Box2D WASM still refuses `totalEntityCount > 65535` bodies — that is a different campaign. Existing demos stay on 16 unless the scene opts in (bunnyMark is 32-wide, 250k).
...
1. Latch published physics pose (`poseSync` / `poseDataA/B`) once — entities, adobe, shadows, parented deco compose from that snapshot (boot: live `Transform` if `ready===0`). `preRender.interpolation: true` blends the last two physics poses while packing. That is not display-rate smoothing. `preRender.fixedFps` slows this whole worker; pair it with `renderer.interpolation` if sprites should keep moving between publishes.
...
2. If new: switch read buffer to `(readyFrame - 1) % 2`, latch `renderQueueCamera`. `Atomics.store(renderQueueSync, 1, readyFrame)` + notify runs after shadows, sprites, and custom layers have read that buffer. A canvas that is not presenting releases immediately so pre-render does not wait.
...
4. When frame-locked passes run:
   - Sync main sprites (ENTITIES layer) from the main render queue. With `ySort` or a sprite `zIndex`, Pixi keeps last frame's order and reinserts only the slots whose `sortKey` changed, then draws them back to front in one blend. `zIndex` is the coarse band. `ySort` orders by Y inside that band. Particles, decorations, bullets, Adobe pieces, and liquid join that list. Glow stays a later ADD batch on the `lightGlows` layer when `renderer.lightGlow` is `'add'` (the default). `'sprite'` does not create that layer: the glow quads stay in the entity list, a little in front of the body, and composite source-over (the soft edge can go gray, and the lighting multiply sits on top of them). Custom sprite layers use the same `_uploadSortedSprites` / `orderPainterSlots` path. Without `ySort` and with every zIndex at 0, nothing is sorted (bunny). With no painter and lighting off, the upload is dense: no `idxE` walk. The instance vertex buffer is two Pixi buffers swapped each upload, so `bufferSubData` does not hit the buffer the previous draw is still reading. `useZBuffer` (default off) skips that painter when `ySort` or a sprite `zIndex` gives it a key: the same sort key becomes clip Z (`1 - key / (worldHeight * 128)`), the entity batch depth-tests, and texels under `renderer.alphaCut` (default 1/255) do not write Z. The high cut stays 0. Opaque cards belong there; a soft PNG edge still writes Z and hides what is behind it. Glow ADD, shadows, and lighting do not depth-test. The old coverage pass stays off: on a moving 300k grid that two-pass tied the CPU painter (30.6 ms vs 30.1 ms).
