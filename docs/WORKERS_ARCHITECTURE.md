...
- **`config.entityIdWidth` 16 | 32** (default **16**): ECS entity ids in active lists, queries, neighbor/grid slots, and entity Treiber stacks. Width 16 keeps the product ceiling at `MAX_ENTITIES` 65535 (`Uint16`). Width 32 turns on the measured compose **FL1 + PAIR3 + NBR1 + GRID1 + LIST-U32** with ceiling `MAX_ENTITIES_U32` 300000. Particle / decoration / bullet pools stay `Uint16`. Bind once via `bindEntityIdWidth` when SABs are created; do not scatter `=== 32` in hot loops. Box2D WASM still refuses `totalEntityCount > 65535` bodies — that is a different campaign. Existing demos stay on 16; do not ship Balls or bunnyMark at 32 by default.
