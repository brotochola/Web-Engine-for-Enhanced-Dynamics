...
| `RIGID` | `SolveRigid`: slab velocity from COM + ω. Rest vs fixtures is `SolveRigidDamping` (group impulse), not per-particle `SolveCollision` |
...
6. Elastic / spring **late** (after damping; they read current velocities). `LimitVelocity` at `|v| <= diameter/dt`. If any group is `RIGID`, refresh group COM/ω from particle velocities then **`SolveRigidDamping`** (Google 1.1.0): impulse on the **group** (and the Box2D body) from body contacts, plus rigid-vs-other-group particle contacts. **SOLID↔SOLID** is **one** COM impulse per group pair (weight-averaged normal) — per-contact COM damping overshoots and nests ice. Ice vs oil stays per-contact. Per-particle `SolveDamping` is not enough — `SolveRigid` overwrites member velocities with COM+ω. If `BARRIER`, `SolveBarrier` (`tmax = 2.5 * dt`).
7. **`SolveCollision`** — reuses step 2's shared query (same swept-cloud AABB, one `OverlapAABB` per sub-step total, not two), `b2Shape_RayCast(shape, p, dt*v)`. Point particle. `target = lerp(p1,p2,fraction) + B2_LINEAR_SLOP * n` (Weed 100 px/m → 0.5 px). `v = inv_dt * (target - p)`. **No radius offset. Do not write position.** Do **not** `b2World_CastShape` per particle. The search padding (`diameter`) is sufficient because `LimitVelocity` (step 6) already caps `dt·|v| <= diameter` for every particle — same CFL bound closes the loop, not a coincidence. Known gap: `SolveBarrier` runs after `LimitVelocity` and doesn't re-clamp, so a `BARRIER`-paired particle could in principle exceed that bound (unresolved, low-impact — opt-in flag, few particles in practice). Then **`SolveRigid`** writes slab velocity from the (already damped) COM+ω. Do not recompute group stats here — that would wipe `SolveRigidDamping`.
...
| [`tests/node/liquidFun.wasm.test.js`](../tests/node/liquidFun.wasm.test.js) | Y-down floor settle + `spanY`; no wall-climb **and** no centers inside the wall; water beside a thick box (`maxPen < radius`); **SOLID\|RIGID ice rests on a static floor** (no tunnel) **and** ice beside a thick box (`maxPen < radius`); overlapping ice bars eject and **do not nest** (`overlapX < 3 radius`); `sync_active_particle_groups` copies `groupFlags` at `stride * 11`; 10k create/step smoke; **1-particle point rest** on floor top (`|vy|` small); barrier smoke; staticPressure finite; deinterleaved `x`/`y` exactly match interleaved `pos`; `strictContactCheck` 5th-arg smoke |
