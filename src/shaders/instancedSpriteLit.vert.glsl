...
out vec2 vTileInv;
out vec2 vTileOff;
out vec4 vTangent; // tx.xy, ty.xy in world
...
void main() {
  ...
  float sx = aInstScale.x < 0.0 ? -1.0 : 1.0;
  float sy = aInstScale.y < 0.0 ? -1.0 : 1.0;
  if (aInstTileInv.x > 0.0 || aInstTileInv.y > 0.0) {
    vTangent = vec4(1.0, 0.0, 0.0, 1.0);
  } else {
    vTangent = vec4(c * sx, s * sx, -s * sy, c * sy);
  }
}
