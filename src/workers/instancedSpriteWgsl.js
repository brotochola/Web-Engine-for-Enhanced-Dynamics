import { PIXI_MESH_GPU_UNIFORMS, gpuStageF, gpuStageVF } from './pixiMeshWgsl.js';
...
export const INSTANCED_SPRITE_WGSL = `${PIXI_MESH_GPU_UNIFORMS}
...
export const LF_SPLAT_WGSL = `${PIXI_MESH_GPU_UNIFORMS}
...
export function instancedSpriteGpuProgram(GpuProgram, fragEntry, name) {
  const vf = gpuStageVF();
  const f = gpuStageF();
  return GpuProgram.from({
    name,
    vertex: { source: INSTANCED_SPRITE_WGSL, entryPoint: 'mainVert' },
    fragment: { source: INSTANCED_SPRITE_WGSL, entryPoint: fragEntry },
    layout: {
      0: { globalUniforms: 0 },
      1: { localUniforms: 0 },
