...
/** Lit sprite: color + normal map + light data texture. */
export function instancedSpriteLitGpuProgram(GpuProgram, source, fragEntry, name) {
  if (!source) {
    throw new Error(
      'WeedJS: Instanced sprite lit WGSL was not loaded before GpuProgram creation.'
    );
  }
  const vf = gpuStageVF();
  const f = gpuStageF();
