...
/**
 * Rewrite pose/paint for entities in `entityIds`. Each entity's tris are a
 * contiguous range built by `buildMeshInstanceRanges`. Other instances stay.
 * @returns {number} instances rewritten
 */
export function packColliderFillPoseOnlyEntities(
  out,
  cap,
  entityIds,
  entityCount,
  ...
  const n = entityCount | 0;
  if (!out || n <= 0 || !entityIds || !instanceStart || !instanceCount) return 0;
  const outU32 = packOutU32(out, views);
  const offsetX = views.offsetX;
  const offsetY = views.offsetY;
  const meshTint = views.meshTint;
  ...
  let rewritten = 0;
  for (let m = 0; m < n; m++) {
    const i = entityIds[m] | 0;
    const count = instanceCount[i] | 0;
    if (count <= 0) continue;
    const begin = instanceStart[i] | 0;
    bindMeshPaint(i, views);
    const pose = readMeshFillPose(i, views, meshFillUsesLive(i, views));
    const c = pose.c;
    const s = pose.s;
    ...
    const a8 = (a * 255 + 0.5) | 0;
    const tint = ((a8 & 255) << 24) | ((meshTint[i] >>> 0) & 0xffffff);
...
/** Contiguous instance ranges from a full pack. Returns false if an entity is split. */
export function buildMeshInstanceRanges(instanceEntity, packed, instanceStart, instanceCount) {
  const n = packed | 0;
  instanceStart.fill(0);
  instanceCount.fill(0);
  for (let k = 0; k < n; k++) {
    const e = instanceEntity[k] | 0;
    const have = instanceCount[e] | 0;
    if (have === 0) instanceStart[e] = k;
    else if ((instanceStart[e] | 0) + have !== k) return false;
    instanceCount[e] = have + 1;
  }
  return true;
}
