...
class PixiRenderer extends AbstractWorker {
  ...
  /**
   * Report visible units count to main thread (throttled)
   */
  reportVisibleUnits(count) {
    const now = Date.now();

    // Only report if count changed or enough time has passed
    if (
      count !== this.lastReportedVisibleCount &&
      now - this.lastVisibleUnitsReportTime > this.visibleUnitsReportInterval
    ) {
      self.postMessage({
        msg: "visibleUnits",
        count: count,
      });
      this.lastReportedVisibleCount = count;
  ...
  updateSprites() {
    ...
    // Report visible count to main thread (throttled)
    this.reportVisibleUnits(visibleCount);
  }
