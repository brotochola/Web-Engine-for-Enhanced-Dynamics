...
class PhysicsWorker extends AbstractWorker {
  ...
  moveBallsVerlet(
    ...
    // Get damping factor from config (0-1, where 1 = no damping, 0.99 = slight damping)
    const damping = this.config.physics?.verletDamping || 0.995;

    // Gravity scaling factor (use squared time for smooth acceleration)
  ...
  applyConstraintsVerlet(
    ...
    // Reset collision counts at the start of constraints step
    // This ensures we get an accurate count of entity/entity contacts per frame
    for (let i = 0; i < this.entityCount; i++) {
      if (!active[i]) continue;
      collisionCount[i] = 0;
    }

    // Get boundary elasticity from config (0-1, where 0 = sticky, 1 = perfectly elastic)
    const boundaryElasticity = this.config.physics?.boundaryElasticity || 0.8;
  ...
  resolveCollisionsVerlet(active, x, y, radius, collisionCount) {
    ...
    // Increased default to 1.0 to prevent crushing in stacks
    const responseStrength =
      this.config.physics?.collisionResponseStrength || 1.0;
  ...
  updateDerivedProperties(active, x, y, px, py, vx, vy, velocityAngle, speed) {
    // Minimum speed threshold for angle updates (prevents jitter when stationary)
    const minSpeedForRotation = this.config.physics?.minSpeedForRotation || 0.1;
