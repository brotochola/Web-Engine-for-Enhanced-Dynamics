...
class GameEngine {
  ...
  constructor(config, imageUrls) {
    ...
    this.config = {
      resolveCollisions: false, // Enable physics-based collision resolution
  ...
  async createWorkers() {
    ...
    // Flatten nested config structure for workers (physics and spatial)
    const flattenedConfig = {
      ...this.config,
      // Flatten physics config
      ...(this.config.physics || {}),
      // Flatten spatial config
      ...(this.config.spatial || {}),
      // Flatten logic config (if it exists)
      ...(this.config.logic || {}),
      // Extract noLimitFPS settings for each worker type to avoid conflicts
      physicsNoLimitFPS: this.config.physics?.noLimitFPS || false,
      spatialNoLimitFPS: this.config.spatial?.noLimitFPS || false,
      logicNoLimitFPS: this.config.logic?.noLimitFPS || false,
    };
