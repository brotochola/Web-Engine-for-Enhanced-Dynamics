  ...
  _autoRegisterParentClasses(EntityClass) {
    ...
    for (const ParentClass of parentChain) {
      ...
      if (!alreadyRegistered && ParentClass !== EntityClass) {
        ...
        // Determine script path for parent class
        // Try to infer from the class name (e.g., Boid -> boid.js)
        const inferredScriptPath = `${ParentClass.name.toLowerCase()}.js`;

        this.registeredClasses.push({
          class: ParentClass,
          count: 0,
          startIndex: startIndex,
          scriptPath: inferredScriptPath,
        });
