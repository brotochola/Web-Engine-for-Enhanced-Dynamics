import { A as AbstractWorker, G as GameObject } from './AbstractWorker.js';
...
class PhysicsWorker extends AbstractWorker {
    ...
    initialize(data) {
        ...
        console.log(`PHYSICS WORKER: Sub-steps per frame: ${this.settings.subStepCount}`);
        console.log(`PHYSICS WORKER: Ready to integrate ${this.entityCount} entities`);
        console.log("PHYSICS WORKER: Initialization complete, waiting for start signal...");
    ...
    applyPhysicsConfig(partialConfig = {}) {
        ...
        this.settings = {
            ...this.settings,
            subStepCount: Math.max(1, source?.subStepCount ?? this.settings.subStepCount),
            boundaryElasticity: clamp01(source?.boundaryElasticity ?? this.settings.boundaryElasticity, this.settings.boundaryElasticity),
            collisionResponseStrength: clamp01(source?.collisionResponseStrength ??
                this.settings.collisionResponseStrength, this.settings.collisionResponseStrength),
            verletDamping: clamp01(source?.verletDamping ?? this.settings.verletDamping, this.settings.verletDamping),
            minSpeedForRotation: source?.minSpeedForRotation ?? this.settings.minSpeedForRotation,
            gravity: {
                x: source?.gravity && typeof source.gravity.x === "number"
                    ? source.gravity.x
                    : this.config.gravity?.x ?? this.settings.gravity.x ?? 0,
                y: source?.gravity && typeof source.gravity.y === "number"
                    ? source.gravity.y
    ...
    updateVerlet(deltaTime, dtRatio) {
        ...
        this.moveBallsVerlet(active, x, y, px, py, vx, vy, ax, ay, dtRatio, gx, gy, maxVel, radius);
        // Step 2: Apply constraints (collisions, boundaries) with sub-stepping
        for (let step = 0; step < this.settings.subStepCount; step++) {
            this.applyConstraintsVerlet(active, x, y, radius, collisionCount, worldWidth, worldHeight);
        }
        // Step 3: Update derived properties (velocityAngle, speed) from positions
        this.updateDerivedProperties(active, x, y, px, py, vx, vy, velocityAngle, speed);
    }
    /**
     * Move balls using Verlet integration
     * ENHANCED: Now includes configurable damping for energy dissipation
     */
    moveBallsVerlet(active, x, y, px, py, vx, vy, ax, ay, dtRatio, gx, gy, maxVel, radius) {
     ...
     */
    applyConstraintsVerlet(active, x, y, radius, collisionCount, worldWidth, worldHeight) {
...
new PhysicsWorker(self);
//# sourceMappingURL=physics_worker.js.map
