...
class SpatialWorker extends AbstractWorker {
    ...
    initialize(data) {
        ...
        console.log("SPATIAL WORKER: Initialization complete, waiting for start signal...");
    ...
    findNeighborsForEntity(i, x, y, visualRange, radius) {
        if (!this.grid || !this.neighborData || !this.distanceData)
            return;
        const myX = x[i];
        const myY = y[i];
        ...
        for (let rowOffset = -cellRadius; rowOffset <= cellRadius; rowOffset++) {
            for (let colOffset = -cellRadius; colOffset <= cellRadius; colOffset++) {
                ...
                for (let k = 0; k < cellLength; k++) {
                    ...
                    if (neighborCount >= this.maxNeighborsPerEntity)
                        break;
    ...
    updateScreenVisibility() {
        ...
        for (let i = 0; i < this.entityCount; i++) {
            ...
            const onScreen = screenX > -marginX &&
...
new SpatialWorker(self);
//# sourceMappingURL=spatial_worker.js.map
